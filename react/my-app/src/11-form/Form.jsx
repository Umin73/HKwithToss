...
/*
< 리액트 입력폼에 대한 개념 > 
리액트에서 입력폼(input form)은 사용자의 입력을 받아 상태(state)로 관리하는 중요한 요소
일반적으로 입력폼을 만들 때는 리액트의 상태(state)와 이벤트 핸들러를 사용하여 입력 데이터를 관리

리액트 폼은 크게 두 가지로 나뉨
1. 제어 컴포넌트(Control Component): 입력값이 리액트 상태에 의해 제어되는 컴포넌트
2. 비제어 컴포넌트(Uncontrolled Component): 입력값이 DOM 노드에 의해 관리되고 상태로 제어하지 않는 방식
...
export function ControlledForm() {
  ...
  return (
    <>
      <form action="#" obSubmit={handleSubmit}>
        <div>
          <label>이름: </label>
          <input
            type="text"
