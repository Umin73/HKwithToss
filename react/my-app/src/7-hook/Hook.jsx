...
// useMemo 훅
// useMemo는 특정 연산의 결과를 Memorization하여
// 불필요한 재계산을 방지하는데 사용된다
// 컴포넌트가 렌더링 될 때 마다
// 의존성 배열에 명시된 값이 변경될 때만 해당 연산을 다시 수행
// 용도
...
export function Counter4() {
  ...
  // number 상태변수가 변경되지 않는한, double값은 재연산되지 X
...
export function Counter5() {
  ...
  // useCallback으로 숫자 입력 핸들러
  // e: JS 이벤트 객체, 이벤트 함수에서 전달됨
  const handleNumberChange = useCallback(
    (e) => {
      console.log("useCallback Memoization1");
      setNumber(parseInt(e.target.value));
    },
    [number]
  );
  ...
  const handleInputChange = useCallback(
    (e) => {
      console.log("useCallback Memoization2");
      setInputValue(e.target.value);
    },
    [inputValue]
  );
...
export function Counter6() {
  ...
  // clickCountRef는 컴포넌트가 리렌더링 될 때 값이 초기화 되지 않음
  // count는 리렌더링 될 때 화면에 즉시 반영하지만
  // clickCountRef는 리렌더링을 발생시키지 않고 값만 유지
  const handleClick = () => {
    setCount(count + 1); // 화면에 리렌더링
    clickCountRef.current += 1; //
    console.log("총클릭횟수: ", clickCountRef.current);
  };
...
export function Counter7() {
  ...
  const handleClick = () => {
    setCount(count + 1); // 상태변경은 리액트에서 비동기적으로 이루어지므로, 바로+1
    clickCountRef.current += 1;
    console.log("총 클릭 횟수: ", clickCountRef);
